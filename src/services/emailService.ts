...
// Brevo API Configuration (using working API key from jayjchiringz@gmail.com account)
...
export const testEmailConnection = async (): Promise<{
  ...
  // Test the API key by making a simple account info request
  const options = {
    hostname: 'api.brevo.com',
    port: 443,
    path: '/v3/account',
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ...
          resolve({
            success: false,
            message: `API key invalid: ${res.statusCode}`,
          });
        }
      });
    });
